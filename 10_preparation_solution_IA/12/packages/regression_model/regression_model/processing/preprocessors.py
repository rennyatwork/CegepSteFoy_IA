...
class CategoricalEncoder(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        for var in self.variables:
            # Replace 'None' with a string indicating missing category
            X[var] = X[var].replace('None', 'Missing')
    ...
    def transform(self, X):
        ...
        for feature in self.variables:
            # Replace 'None' with a string indicating missing category
            X[feature] = X[feature].replace('None', 'Missing')

            # Check if category exists in the encoder dictionary
            X[feature] = X[feature].map(lambda x: self.encoder_dict_[feature].get(x, -1))
